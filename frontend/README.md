...
2. `js/game.js` の `TetrisGame` クラス各メソッドを、`docs/06_研修手順書.md`
   「5. Day3：フロントエンド実装①」のSTEP 3-1〜3-10の順に、docstringコメントと
   詳細設計書4章（回転仕様）・5章（得点計算仕様）に従って実装する
   （`TETROMINO_SHAPES` などのテトリミノ形状データ、および `start`/`stop`/`_spawnPiece`/
   `_loop`/`_handleKeydown`/`_render` は実装済みなのでそのまま使える。
   `_renderBoard`/`_renderNext`/`_drawCell` はSTEP 3-1で実装する）
3. `js/api.js` に進む（`docs/06_研修手順書.md` 「6. Day4：フロントエンド実装②」STEP 4-1〜4-6。
   各関数のdocstringコメントに対応するAPI仕様（A-01〜A-07）が書かれているので、
   `fetch` でバックエンドを呼び出す処理を実装する）
